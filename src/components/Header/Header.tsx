...
export default function Header() {
  ...
  const logoutMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setProfile(null)
      queryClient.removeQueries({ queryKey: ['cart', { status: status.inCart }] })
    }
  })
  ...
  const { data: cartData } = useQuery({
    queryKey: ['cart', { status: status.inCart }],
    ...
    <div className='pb-5 pt-2 bg-[linear-gradient(-180deg,#f53d2d,#f63)] text-white'>
      <div className='container'>
        ...
        <div className='grid items-end grid-cols-12 gap-4 mt-4'>
          ...
          <Link to='/cart' className='col-span-1 justify-self-end'>
            <Popover
              renderPopover={
                <div className='relative text-sm bg-white border border-gray-200 rounded-sm shadow-md'>
                  <div className='p-2 min-h-[200px] min-w-[500px] text-sm flex flex-col'>
                    <div className='capitalize htext-gray-400'>Sản phẩm mới thêm</div>
                    <div className='mt-5'>
                      {cartData &&
                        cartData?.data.data.length > 0 &&
                        cartData?.data.data.map((item, index) => (
                          <div className='flex mt-4' key={item._id}>
                            ...
                            <div className='flex-shrink-0 ml-2'>
                              <span className='text-orange'>₫{formatCurrencyVND(item.product.price)}</span>
                            </div>
                          </div>
                        ))}
                      {cartData && cartData.data.data.length <= 0 && (
                        <div className='py-4 text-center'>Chưa có sản phẩm</div>
                      )}
                    </div>
